package com.example.app_ui.common.view.floorsview
...
import kotlinx.android.synthetic.main.viewholder_floor.view.*
...
class FloorsAdapter: RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return ItemHolder(
                LayoutInflater.from(parent.context).inflate(R.layout.viewholder_floor, parent, false)
        )
    }
        ...
        private fun setColor(selected: Boolean) {
            with(itemView) {
                if (selected) {
                    ...
                    tvFloor.setBackgroundColor(ContextCompat.getColor(context, R.color.colorBack))
                }
            }
        }
    }
